...
t(/^<div class="fdPanel wdFull dense">/.test(html) && /<\/div><div class="wdTiles">[\s\S]*<\/div><\/div><\/div>$/.test(html) && !/fdFoot/.test(html), 'the sheet stands full height (dense, six things or more) and the tiles are the last thing on it');
