...
const MINE=['_CI_PILLS','_ciState','_ciText','ciType','_ciPillsHtml','_ciPillsPaint','ciPill','_ciTodayHtml','_tlCheckinBlock','_ciOpen','_tlAskRow'];
global._ciGrow=()=>{}; global._stepsFor=()=>null; global._tlHasCardio=()=>false;
const CL=closure(MINE); eval(CL.code||''); guard(MINE, n=>eval(n));
_fdShow=(html)=>{ global.__sheet=html; return true; };
const src=fs.readFileSync('index.html','utf8');
cl={code:'zzscratchnotaclient'};

console.log('\n  THE ROW, AND THE SHEET IT OPENS (11 Sep: "something should be better about check in"):');
const row=_tlCheckinBlock('Sep 10, 2026', true, false);
t(/tlMealsEy">Check in</.test(row) && /data-tl="checkin"/.test(row) && /tlMealAskT">Check in</.test(row) && !/ciBox|ciSaveAll/.test(row), 'on the page it is one plus row under the CHECK IN head - no open box, no Save');
_ciOpen();
let h=global.__sheet;
t(/if\(a==='checkin'\)\{ ev\.stopPropagation\(\); try\{ _ciOpen\(\); \}catch\(e\)\{\} return; \}/.test(src), 'the row is wired to the sheet');
t(!/ciTitle/.test(h) && /fdTitle">Check in</.test(h) && /class="ciSheetBody"/.test(h), 'the sheet is the glass sheet every other overlay uses, titled once');
