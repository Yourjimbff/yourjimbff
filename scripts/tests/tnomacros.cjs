...
const writer=src.slice(src.indexOf('async function logFoodFromChat('), src.indexOf('async function logFoodFromChat(')+9000);
...
t(/I could not put numbers on ' \+ _nmName \+ ', so it is not on your day yet\. About how many calories was it, or tell me the size/.test(block),
  'and ASKS for the numbers - the question that actually moves it');
