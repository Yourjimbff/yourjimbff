...
t(!/how it lands/.test(h3), 'no header on a clean draft - the bubbles are the message (7 Sep, the very simple CRM)');
const long=_dfEdHtml('7', Array(30).fill('word').join(' '));
t(/class="dfEdR long"/.test(long), 'over 25 words the bubble goes red');
t(/1 text too long/.test(long), 'and a header appears only to say how many are too long');
...
t(/1 text too long/.test(_dfEdHtml('7', Array(30).fill('word').join(' '))) && /2 texts too long/.test(_dfEdHtml('7', Array(30).fill('word').join(' ')+'\n\n'+Array(30).fill('word').join(' '))), 'text vs texts');
