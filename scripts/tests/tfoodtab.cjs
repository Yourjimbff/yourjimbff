...
console.log('\n  AND THE BAR READS IN HIS ORDER:');
[['bnFeed',1,'Day'],['bnJarvis',2,'Jarvis'],['bnProgram',3,'Program'],['bnFood',4,'Food'],['bnClients',5,'Feed']]
  .forEach(function(r){
    t(new RegExp('\\.bnav\\.trainer-mode #'+r[0]+'\\{order:'+r[1]+';\\}').test(src), '  '+r[1]+'. '+r[2]);
  });
/* Nothing else may hold one of those five places, or two buttons share a column
   and one of them is drawn on top of the other. */
const orders=(src.match(/\.bnav\.trainer-mode #bn[A-Za-z]+\{order:\d+;\}/g)||[]);
const nums=orders.map(function(o){ return o.match(/order:(\d+)/)[1]; });
t(new Set(nums).size===nums.length, 'no two buttons claim the same slot', orders);
